...
/*
function PLGMainInit(isAdmin) {

var col0 = (isAdmin ? "#C3D6F7" : "#e6e6e6");
var col1 = (isAdmin ? "#F3F6FF" : "#f4f4f4");

var list = $$('.PLGMain TR');
list.each(function(element) {
if (element.className == 'PLGHeaderStyle' || element.className == 'PLGPagerStyle') return;
...
element.addClass('PLGOnMouseItemStyle');
});
element.addEvent('mouseleave', function() {
fx.start({
'background-color': (element.className.indexOf('PLGAlternatingItemStyle') != -1 ? col1 : '#FFFFFF')//,
});
element.removeClass('PLGOnMouseItemStyle');
});
});
...
VikiAdminFile.Init = function() {
    $('[viki-admin-file] > .viki-upload').bind('click', function() {
    ...
    $('[viki-admin-file] > input').bind('change', function() {
        var block = $(this).parents('[viki-admin-file]:first');
        var bl_img = block.find('img')[0];
        var bl_a = block.find('a[class!=viki-upload]')[0];
        var bl_type = block.attr('viki-admin-file');
