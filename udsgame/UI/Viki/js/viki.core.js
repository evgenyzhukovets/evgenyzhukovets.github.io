...
Viki.SetupLive = function (selector) {
  ...
  /*$(selector).find('a[viki-ajax-type]:not([viki-action])').live('click', {}, function (e) { return Viki.AjaxLink(this, e); });*/
...
Viki.SetupPartial = function (selector) {
  ...
  $(selector).find('[viki-keypress]').bind("keydown", function (e) { return Viki.KeyPress.KeyUp($(this), e); })
...
Viki.Tabs.Init = function (selector) {
  $(selector).find('.viki-tabs [data-viki-tabs]').live('click', {}, function (e) {
...
Viki.KeyPressDdl.Init = function (selector) {
  $(selector).find('[viki-keypress-ddl]')
        .bind("keydown", function (e) { return Viki.KeyPressDdl.KeyUp($(this), e); })
...
Viki.DataPicker.Init = function () {
  ...
  $(".viki-datepicker").live('focusin', {}, function (e) { return Viki.DataPicker.Focus($(this), e); });
};
Viki.DataPicker.Focus = function (sender, e) {

  if (!$(sender).is(':data(datepicker)')) {
...
Viki.Watermark = function () { };
Viki.Watermark.Init = function (selector) {
  $(selector).find('input:[viki-watermark]').live('focusin', {}, function (e) {
    var mess = $(this).attr('viki-watermark');
    if ($.trim($(this).val()) === mess) { $(this).val(''); };
  });
  $(selector).find('input:[viki-watermark]').live('focusout', {}, function (e) {
    Viki.Watermark.IsEmpty($(this));
  });
};
Viki.Watermark.IsEmpty = function (it) {
  var mess = it.attr('viki-watermark');
  var ret = ($.trim(it.val()) === '' || $.trim(it.val()) === mess);
  if (ret) it.val(mess);
  return ret;
};
